...
DMLPact currently has one repository owner and one release-capable maintainer,
...
- the released native binary by running `dmlpact --version`.
...
GH_TOKEN="$(gh auth token)" \
  ./scripts/continuity-drill.sh yhay81/dmlpact dmlpact
```
...
The public drill reduces data-recovery uncertainty but does not satisfy
DMLPact's two-maintainer v1.0 requirement. Two active, release-capable
maintainers must independently exercise the release and mutation-incident
runbooks before v1.0.
